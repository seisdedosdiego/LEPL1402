...
public class KNN {
    ...
    static class StudDist {
        Student student;
        double distance;

        public StudDist(Student student, double distance) {
    ...
    public static boolean predictSuccess(Student [] students, double[] grades, int k) {
        StudDist[] allStudDists = new StudDist[students.length];
        for (int i=0 ; i<students.length ; i++) {
            double distance = euclideanDistance(students[i].grades, grades);
            allStudDists[i] = new StudDist(students[i], distance);
        }
        Arrays.sort(allStudDists, Comparator.comparingDouble(sd -> sd.distance));
        int successCount = 0;
        for (int i=0 ; i<k ; i++) {
            if (allStudDists[i].student.success) {
                successCount++;
            }
        }
