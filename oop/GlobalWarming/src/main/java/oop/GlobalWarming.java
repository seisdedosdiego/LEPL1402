...
public class GlobalWarming {
    ...
    static public class IPCCDatabase {
        ...
        private Map<String, Float> temperatureRecords = new HashMap<>();
        ...
        public void temperatureMeasured(String place, float temperature) {
            if (!temperatureRecords.containsKey(place)) {
                temperatureRecords.put(place, temperature);
                notifyObservers(place, temperature);
                return;
            }
            float currentRecord = temperatureRecords.get(place);
            if (temperature > currentRecord) {
                temperatureRecords.put(place, temperature);
                notifyObservers(place, temperature);
            }
