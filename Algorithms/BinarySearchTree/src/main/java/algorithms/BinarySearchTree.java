...
public class BinarySearchTree {
    ...
    public List<Integer> decreasing() {
        ...
        traversInDescendingOrder(root, result);
        return result;
    }

    private void traversInDescendingOrder(Node node, List<Integer> result) {
        if (node == null) return;
        traversInDescendingOrder(node.right, result);
        result.add(node.key);
        traversInDescendingOrder(node.left, result);
    }
