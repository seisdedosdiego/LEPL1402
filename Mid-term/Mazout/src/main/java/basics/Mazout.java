...
public class Mazout {
    ...
    public static int getTotalDemand(City city, House firstHouse, String[] path) {
        ...
        int[] visitedHouses = new int[city.getNumberOfHouses()];
        House currentHouse = firstHouse;
        if (currentHouse == null) {
            throw new IllegalArgumentException("Nonexistent first house");
        }
        int currentHouseIndex = firstHouse.getIndex();
        totalFuelNeeded += currentHouse.getCapacity();
        visitedHouses[currentHouseIndex] = 1;
        for (int i=0 ; i<path.length ; i++) {
            ...
            currentHouseIndex = currentHouse.getIndex();
            if  (visitedHouses[currentHouseIndex] == 0) {
                totalFuelNeeded += currentHouse.getCapacity();
                visitedHouses[currentHouseIndex] = 1;
            }
        }
