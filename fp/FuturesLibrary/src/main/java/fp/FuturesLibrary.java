...
public class FuturesLibrary {
    ...
    public static int countMatchingBooksWithThreads(Library library,
        ...
        int totalBooks = library.getNumberOfBooks();
        int booksPerThread = totalBooks/countThreads;
        int remainingBooks = totalBooks%countThreads;
        ...
        for (int i=0; i<countThreads; i++) {
            int booksToProcess = booksPerThread + (i < remainingBooks ? 1 : 0);
            int threadStartIndex = startIndex;
            int threadBooksCount = booksToProcess;

            Future<Integer> future = executor.submit(() -> {
                return countMatchingBooks(library, predicate, threadStartIndex, threadBooksCount);
            });
            futures.add(future);
            startIndex += booksToProcess;
        }
