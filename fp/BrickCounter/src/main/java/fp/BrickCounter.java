...
public class BrickCounter {
    ...
    public static int[] countBricksTwoThreads(Brick[] bricks, int n, Function<Brick, Integer> sorter,
        ...
        try {
            int mid = bricks.length / 2 ;

            Future<int[]> firstHalFuture = executor.submit(() -> {
                int[] res = new int[n];
                for (int i = 0; i<mid; i++) {
                    int num = sorter.apply(bricks[i]);
                    res[num]++; 
            ...
            Future<int[]> secondHalFuture = executor.submit(() -> {
                int[] res = new int[n];
                for (int i = mid; i<bricks.length; i++) {
                    int num = sorter.apply(bricks[i]);
                    res[num]++; 
                }
                 return res;
            ...
            int[] firstHalf = firstHalFuture.get();
            int[] secondHalf = secondHalFuture.get();

            int[] combinedResult = new int[n];

            for(int i = 0; i<n; i++) {
                combinedResult[i] = firstHalf[i] + secondHalFuture.get()[i];
            }
