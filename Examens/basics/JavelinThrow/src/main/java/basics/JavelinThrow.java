...
public class JavelinThrow {
    ...
    public static JavelinPosition findJavelin(char[][] field) {
        ...
        for (int x=0; x<fieldHeight; x++) {
            for (int y=0; y<fieldWidth; y++) {
                if (field[x][y] == '#') {
                    return new JavelinPosition(y,x);
                }
            }
        }
    ...
    public static boolean isThrowValid(char[][] field) {
        ...
        int linePosition = jp.y;
        int colomnPosition = jp.x;   // problème de variables ici : inversion

        int lastBoundarie = -1;
        for (int i=0; i<fieldWidth; i++) {
            if (field[linePosition][i] == 'x' || field[fieldHeight-linePosition-1][i] == 'x') {
                lastBoundarie = i;
            } else if (i<fieldWidth-1 && field[linePosition][i] == '#' && field[linePosition][i+1] == 'x') {
                lastBoundarie = i;
            }
        }

        System.out.println(lastBoundarie);
        System.out.println(colomnPosition);

        if (lastBoundarie < colomnPosition) {
            return true;
        } else { 
            return false;
        }
