...
public class Mazout {
    ...
    public static int getTotalDemand(City city, House firstHouse, String[] path) {
        ...
        House first = firstHouse;
        if (first == null) {
            throw new IllegalArgumentException("First house is null.");
        }
        House currentHouse = firstHouse;
        int currentIndex = first.getIndex();

        total += first.getCapacity();
        visited[currentIndex] = true;

        for (int i=0; i<path.length; i++) {
            String direction = path[i];
            if (path[i] == null) {
                throw new IllegalArgumentException("This path is null.");
            }
            switch (direction) {
                case "left":
                ...
                case "down":
                    ...
                    break;
                default:
                    throw new IllegalArgumentException("This path is invalid.");
            }
            if (currentHouse == null) {
                throw new IllegalArgumentException("This path lead us to a null house.");
            }
            currentIndex = currentHouse.getIndex();
            if (visited[currentIndex] == false) {
                total += currentHouse.getCapacity();
                visited[currentIndex] = true;
            }
        }
