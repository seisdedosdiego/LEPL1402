...
public class GlobalWarming {
    ...
    static public class IPCCDatabase {
        ...
        private HashMap<String,Float> pastRecords = new HashMap<>();
        // Hint 2: You will need to store the observers in some data structure.
        private ArrayList<RecordObserver> observers = new ArrayList<>();
        ...
        public void temperatureMeasured(String place, float temperature) {
            if (!pastRecords.containsKey(place)) {
                pastRecords.put(place, temperature);
                NotifyObservers(place, temperature);
            } else {
                if (pastRecords.get(place) < temperature) {
                    pastRecords.put(place,temperature);
                    NotifyObservers(place, temperature);
                }
            }
        ...
        private void NotifyObservers(String place, float temperature) {
            for (RecordObserver obs : observers) {
                obs.signalNewRecord(place, temperature);
            }
        }
    }
}