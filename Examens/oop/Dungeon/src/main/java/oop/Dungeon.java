...
public class Dungeon {
    ...
    class HealthFountain {
        int hf;
        HealthFountain(int hf) {
            this.hf = hf;
        }
    }
    ...
    class Door {
        ...
        Door (Dungeon otherDungeon) {
            this.otherDungeon = otherDungeon;
        }
    }
    ...
    **/
    Dungeon(int dimensionX, int dimensionY) {
        grid = new Object[dimensionY][dimensionX];
    }
    ...
    */
    ...
    void putHealthFountain(int x, int y, int hf) {
        grid[y][x] = new HealthFountain(hf);
    }
    ...
    void putDoor(int x, int y, Dungeon otherDungeon) {
        grid[y][x] = new Door(otherDungeon);
    }
    ...
    public static void movePlayer(Player player, Direction direction) {
        ...
        if (player.dungeon.grid[py][px] instanceof Trap) {
            player.healthPoints--;
        } else if (player.dungeon.grid[py][px] instanceof HealthFountain) {
            HealthFountain fountain = (HealthFountain) player.dungeon.grid[py][px];
            player.healthPoints += fountain.hf;
        } else if (player.dungeon.grid[py][px] instanceof Door) {
            Door door = (Door) player.dungeon.grid[py][px];
            player.dungeon = door.otherDungeon;
            player.y = 0;
            player.x = 0;
        }
