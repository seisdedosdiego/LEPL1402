        ...
        Collections.sort(list, new Comparator<Pair>() {
            ...
            public int compare(Pair p1, Pair p2) {
                if (p1.first != p2.first) {
                    return (int) Math.signum(p1.first-p2.first);  // Tri par le premier élément
                } else {
                    return (int) Math.signum(p1.second-p2.second);  // Tri par le second élément si les premiers sont égaux
                }
            }
        });
        int median = n/2; 
