...
public class BinarySearchTree {
    ...
    public List<Integer> decreasing() {
        ...
        collectInDecreasingOrder(root, result);
        return result;
    }

    public void collectInDecreasingOrder(Node node, List<Integer> result) {
        if (node == null) {
            return;
        }
        collectInDecreasingOrder(node.right, result);
        result.add(node.key);
        collectInDecreasingOrder(node.left, result);
    }
