...
public class JavelinThrow {
    ...
    public static JavelinPosition findJavelin(char[][] field) {
        ...
        return new JavelinPosition(-1,-1); // no Javelin found
    }
    ...
    public static boolean isThrowValid(char[][] field) {
        ...
        JavelinPosition coordinate = findJavelin(field);
        int x = coordinate.x;
        int y = coordinate.y;

        int boundary = -1;
        for (int i=0 ; i<field[fieldHeight-y-1].length ; i++) {
            if (field[fieldHeight-y-1][i] == 'x') {
                boundary = i;
            }
        }
