...
public class Pascal {
    ...
    public static int[] pascal(int n) {
        ArrayList<int[]> triPascal = new ArrayList<>();
        ...
        for (int i=0 ; i<n ; i++) {
            ...
            for (int j=0 ; j<=i ; j++) {
                if (j==0 || j==i) {
                    ...
                    row[j] = triPascal.get(i-1)[j-1] + triPascal.get(i-1)[j];
                }
            }
            triPascal.add(row);
        }
        System.out.println(Arrays.toString(triPascal.get(n - 1)));
        return triPascal.get(n-1);
    }
}
