...
public class ExamGrader {
    ...
    public static int calculateExamGrade(ExamQuestion questions, RoundingFunction roundingFunction) {
        ...
        while (currentQuestion != null) {
            sumOfPoints += currentQuestion.getPointsObtained();
            currentQuestion = currentQuestion.getNextQuestion();
        }
         return roundingFunction.roundGrade(sumOfPoints);
    }
    ...
    public static int[] gradeExams(ExamQuestion exam1, ExamQuestion exam2, RoundingFunction roundingFunction) {
        ...
        try {
            Future<Integer> grade1Future = threadPool.submit(() -> calculateExamGrade(exam1, roundingFunction));
            Future<Integer> grade2Future = threadPool.submit(() -> calculateExamGrade(exam2, roundingFunction));

            grades[0] = grade1Future.get();
            grades[1] = grade2Future.get();
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
            threadPool.shutdown();
